...
Page({
  data: {
    ...
    orderby: "id",
    order: "asc",
    categories: 130, //重要！定义要显示本地活动的类目
  ...
  onShareAppMessage: function () {
    return {
      title: "建始本地优惠活动", //重要！定义要显示分享该页面的标题
      ...
      if (response.statusCode === 200) {
        ...
        self.setData({
          ...
          postsList: self.data.postsList.concat(response.data.map(function (item) {
            ...
            item.actarea= util.cutspit(item.format_quote_content, 0);
            item.location= util.cutspit(item.format_quote_content, 1);
            return item;
          })),
