...
(SELECT post_id from ".$wpdb->postmeta." where meta_value='like' and meta_key='_".$openid."') ORDER BY post_date desc LIMIT 20";        
        ...
        foreach ($_posts as $post) {
            ...
            $custom_author = Raz()->settings()->get_post_meta($post_id, 'custom_author'); //书籍价格
            $custom_author = Raz()->settings()->get_post_meta($post_id, 'custom_author'); //书籍价格
    
            $format_quote_content = Raz()->settings()->get_post_meta($post_id, 'format_quote_content'); //书友推荐内容
            $format_quote_author = Raz()->settings()->get_post_meta($post_id, 'format_quote_author'); //书友推荐人
            $format_quote_background = Raz()->settings()->get_post_meta($post_id, 'format_quote_background'); //内页渐变背景前置
            $format_quote_color = Raz()->settings()->get_post_meta($post_id, 'format_quote_color'); //内页渐变背景后置
            $format_embed_aspect_ration = Raz()->settings()->get_post_meta($post_id, 'format_embed_aspect_ration'); //选择明暗背景
            //**小鱼哥结束**  给文章增加自定义字段 */
