...
function custom_post_fields( $data, $post, $request) { 
  ...
  $_data['listAdEvery']=$listAdEvery;
    ...
    if(!empty($format_embed)){
      $_data["format_embed"] = html_entity_decode($format_embed);
    }
                ...
                $__tmp = '';
                    foreach($ids as $image_id){
                        if(wp_attachment_is_image($image_id)){
                            $__tmp .= sprintf('<div class="galleryimages"><img src="%2$s" alt="" /></div>',
                                get_the_permalink(),
                                Raz()->images()->get_attachment_image_url( $image_id, $thumbnail_size)
                            );
                        }
                    }
                    $_data["format_gallery"] = $__tmp;
